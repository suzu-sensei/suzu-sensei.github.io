...
(function () {
  ...
  const Classroom = {
    ...
    async upcomingScheduleForSlots(studentEmail, slots, count) {
      ...
      // 注意：ここでは「今日、既に元の時間を過ぎているか」で来週に送らない。
      // 振替（override）で元の枠が別の未来日時に移動している場合があり、
      // 元の時間だけで判定すると、その振替後の授業がまだ先なのに見えなくなってしまうため。
      // 「本当に過去かどうか」は、振替を適用した後の実際の日時で後段まとめて判定する。
      ...
      const results = [];
      let guard = 0;
      const guardMax = (count + 60) * slots.length * 3;
      while (results.length < count && guard < guardMax) {
        ...
        let resolvedDate = null, resolvedTime = null, overridden = false, overrideId = null, note = '';
        if (ov) {
          if (ov.new_date) {
            const [ny, nm, nd] = ov.new_date.split('-').map(Number);
            const timeStr = ov.new_time || cur.time;
            const [th, tm] = timeStr.split(':').map(Number);
            resolvedDate = new Date(ny, nm - 1, nd, th || 0, tm || 0, 0, 0);
            resolvedTime = timeStr;
            overridden = true; overrideId = ov.id; note = ov.note || '';
          }
          // ov.new_date が null の場合は「欠席（振替なし）」なのでスキップ（結果に含めない）
        } else {
          resolvedDate = new Date(cur.date);
          resolvedTime = cur.time;
        }
        // 振替後の実際の日時で「まだ先かどうか」を判定してから結果に入れる
        if (resolvedDate && resolvedDate.getTime() > Date.now()) {
          results.push({ date: resolvedDate, time: resolvedTime, overridden, originalDate: iso, overrideId, note });
        }
    ...
    // パックの使用期限（1回目利用日から起算・購入日ではない）を計算。
    // 10回パック＝3ヶ月、20回パック（以上）＝6ヶ月。pack_size未設定の古いデータは10回扱い（3ヶ月）。
    // first_lesson_dateが未設定（まだ1回も消化していない）ならnullを返す＝期限なし。
    packExpiryMonths(student) {
      const size = (student && student.pack_size) || 10;
      return size >= 20 ? 6 : 3;
    },
    packExpiryDate(student) {
      ...
      expiry.setMonth(expiry.getMonth() + this.packExpiryMonths(student));
      const extensionDays = (student && student.expiry_extension_days) || 0;
      if (extensionDays) expiry.setDate(expiry.getDate() + extensionDays);
      return expiry;
    },
